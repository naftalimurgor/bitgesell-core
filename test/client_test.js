...
describe('Client', () => {
  before(async () => {
    const client = new Client(config.bitgesell);
  ...
  describe('constructor', () => {
    it('should throw an error if network is invalid', () => {
      try {
        new Client(_.defaults({ network: 'foo' }, config.bitgesell)); // eslint-disable-line no-new
    ...
    it('should have all the methods listed by `help`', async () => {
      const help = await new Client(config.bitgesell).help();
    ...
    it('should accept valid versions', async () => {
      await new Client(_.defaults({ version: '0.15.0.1' }, config.bitgesell)).getNetworkInfo();
      await new Client(_.defaults({ version: '0.15.0' }, config.bitgesell)).getNetworkInfo();
      await new Client(_.defaults({ version: '0.17.0' }, config.bitgesell)).getNetworkInfo();
    });
  });
  ...
  describe('connections', () => {
    describe('general', () => {
      it('should throw an error if timeout is reached', async () => {
        try {
          await new Client(_.defaults({ timeout: 1 }, config.bitgesell)).listUnspent();
      ...
      it('should throw an error if a connection cannot be established', async () => {
        try {
          await new Client(_.defaults({ port: 9897 }, config.bitgesell)).getDifficulty();
    ...
    describe('ssl', () => {
      it('should use `ssl.strict` by default when `ssl` is enabled', () => {
        const sslClient = new Client(_.defaults({ host: config.bitgesellSsl.host, port: config.bitgesellSsl.port, ssl: true }, config.bitgesell));
      ...
      it('should establish a connection if certificate is self signed but `ca` agent option is passed', async () => {
        const sslClient = new Client(_.defaults({
          ...
          host: config.bitgesellSsl.host,
          port: config.bitgesellSsl.port,
          ssl: true
        }, config.bitgesell));
      ...
      it('should establish a connection if certificate is self signed but `ssl.strict` is disabled', async () => {
        const sslClient = new Client(_.defaults({ host: config.bitgesellSsl.host, port: config.bitgesellSsl.port, ssl: { enabled: true, strict: false } }, config.bitgesell));
        const info = await sslClient.getInfo();
    ...
    describe('authentication', () => {
      it('should throw an error if credentials are invalid', async () => {
        try {
          await new Client(_.defaults({ password: 'biz', username: 'foowrong' }, config.bitgesell)).getDifficulty();
        } catch (e) {
      ...
      it('should support username only authentication', async () => {
        const difficulty = await new Client(config.bitgesellUsernameOnly).getDifficulty();
